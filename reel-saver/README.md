...
## 自分だけ・自分のPCで使う（一番かんたん）

ネット公開は不要で、自分のPCで動かすだけならこれが一番簡単です。

### Windows

1. [Node.js](https://nodejs.org) をインストール（初回のみ）
2. この `reel-saver` フォルダの中の **`start.bat` をダブルクリック**
3. 自動でブラウザが開くので、URL を貼って「保存」を押すだけ

> 2回目以降も `start.bat` をダブルクリックするだけで起動します。ウィンドウを閉じると停止します。
...
ブラウザで <http://localhost:3000> を開けばOK。
