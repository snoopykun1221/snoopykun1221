...
【売れている有料記事に共通する設計（必ず意識すること）】
- 有料記事は情報提供ではなく、読者の「変化」を作るもの。
  読む前と読んだあとで、その銘柄の見え方がどう変わるかを設計してから書く
- 無料部分は「導入と、ものの見方」。有料部分は「具体的な数字と、自分の判断基準」に振り分ける
- 有料部分の最初のセクション（数字で見る、いまの実力）では、
  無料部分では出さなかった具体的な数字をすぐに出す。
  お金を払った直後の数行で「払ってよかった」と思わせること
- 読者が他のどこでも読める一般論を書かない。
  「調べればわかること」ではなく「調べた数字をどう解釈するか」に価値がある"""
